...
ra_lastgame = "storage/.config/lastgame"
ra_lastgame_na = "storage/.config/lastgame_na"
...
def set_settings(rom_name: str, core: str, platform: str, controllers: str, autosave: str, snapshot: str) -> str:
    ...
    if platform in retro_achievements:
        if config.get_setting("retroachievements"):
            ...
            ra_append_dict['cheevos_username'] = config.get_setting(
                "retroachievements.username")
            ra_append_dict['cheevos_password'] = config.get_setting(
                "retroachievements.password")
            ra_append_dict['cheevos_hardcore_mode_enable'] = config.get_bool_string(
                "retroachievements.hardcore")
            ra_append_dict['cheevos_leaderboards_enable'] = config.get_bool_string(
                "retroachievements.leaderboards")
            ra_append_dict['cheevos_verbose_enable'] = config.get_bool_string(
                "retroachievements.verbose")
            ra_append_dict['cheevos_auto_screenshot'] = config.get_bool_string(
                "retroachievements.screenshot")
            ra_append_dict['cheevos_richpresence_enable'] = config.get_bool_string(
                "retroachievements.richpresence")
            ra_append_dict['cheevos_challenge_indicators'] = config.get_bool_string(
                "retroachievements.challenge_indicators")
            ra_append_dict['cheevos_start_active'] = config.get_bool_string(
                "retroachievements.encore")
            ra_append_dict['cheevos_unlock_sound_enable'] = config.get_bool_string(
    ...
    if config.get_setting("netplay"):
        ...
        if value == 'host':
            ...
            ra_append_dict['netplay_ip_port'] = config.get_setting(
                'netplay.port')
        elif value == 'client':
            ra_append_dict['netplay_mode'] = "true"
            ra_append_dict['netplay_ip_address'] = config.get_setting(
                'netplay.client.ip')
            ra_append_dict['netplay_ip_port'] = config.get_setting(
                'netplay.client.port')
        ...
        ra_append_dict['netplay_delay_frames'] = config.get_setting(
            'netplay.frames')
        ra_append_dict['netplay_nickname'] = config.get_setting(
            'netplay.nickname')
        # spectator mode
        ra_append_dict['netplay_spectator_mode_enable'] = config.get_bool_string(
            "netplay.spectator")
        ra_append_dict['netplay_public_announce'] = config.get_bool_string(
    ...
    if config.get_setting('ai_service_enabled'):
        ...
        if ai_url := config.get_setting('ai_service_url'):
            ...
            ra_append_dict[
                'ai_service_url'] = f'http://ztranslate.net/service?api_key=BATOCERA&mode=Fast&output=png&target_lang={ai_lang}'
    else:
    ...
    # Video Integer Scale
    ra_append_dict['video_scale_integer'] = config.get_bool_string(
        "integerscale")

    # Video Integer Scale Overscale
    ra_append_dict['video_scale_integer_overscale'] = config.get_bool_string(
    ...
    # Rewind
    no_rewind = {'sega32x', 'psx', 'zxspectrum', 'odyssey2', 'mame', 'n64',
                 'dreamcast', 'atomiswave', 'naomi', 'neogeocd', 'saturn', 'psp', 'pspminis'}
    ...
    if os.path.isfile(ra_lastgame):
        if config.get_setting('autosave'):
            ...
            ra_append_dict['savestate_auto_load'] = "true"
            ra_append_dict['savestate_auto_save'] = "false"
            with open(ra_lastgame_na, "w") as text_file:
                print(f"no-autosave", file=text_file)

    # Runahead
    # Runahead 1st Instance
    no_run_ahead = {'psp', 'sega32x', 'n64', 'dreamcast',
                    'atomiswave', 'naomi', 'neogeocd', 'saturn'}
    ...
    # Auto Frame Relay
    ra_append_dict['video_frame_delay_auto'] = config.get_bool_string(
    ...
    if core == 'gambatte':
        ...
        if os.path.isfile(gambatte_conf):
            delete_lines(gambatte_conf, ('gambatte_gb_colorization', 'gambatte_gb_internal_palette', 'gambatte_gb_palette_twb64_1',
                                         'gambatte_gb_palette_twb64_2', 'gambatte_gb_palette_twb64_3', 'gambatte_gb_palette_pixelshift_1'))
        else:
        ...
        if not gambatte_gb_colorization:
            ...
            gambatte_gb_internal_palette = config.get_setting(
                'Internal_Palette')
            logger.log(
                f'gambatte internal palette: {gambatte_gb_internal_palette}')
            ...
            logger.log(
                f'gambatte palette twb64 pack 1: {gambatte_gb_palette_twb64_1}')
            ...
            logger.log(
                f'gambatte palette twb64 pack 2: {gambatte_gb_palette_twb64_2}')
            ...
            logger.log(
                f'gambatte palette twb64 pack 3: {gambatte_gb_palette_twb64_3}')
            if gambatte_gb_palette_twb64_3:
                gambatte_dict['gambatte_gb_palette_twb64_3'] = gambatte_gb_palette_twb64_3

            gambatte_gb_palette_pixelshift_1 = config.get_setting(
                'PixelShift_-_Pack_1')
            logger.log(
                f'gambatte palette pixelshift pack 1: {gambatte_gb_palette_pixelshift_1}')
    ...
    if controllers:
        for player in range(1, 6):
            ...
            if pindex := re.search(fr'p{player}index\s+([0-9]+)', controllers):
                ra_append_dict['input_player{player}_joypad_index'] = pindex.group(
                    1)
    ...
    #         #'gb': (80, 24, 480, 432),              # x3
    #         'gb': (53, 0, 534, 480),                # xKEGG
    #         #'gbh': (80, 24, 480, 432),             # x3
    #         'gbh': (53, 0, 534, 480),               # xKEGG
    #         #'gbc': (80, 24, 480, 432),             # x3
    #         'gbc': (53, 0, 534, 480),               # xKEGG
    #         #'gbch': (80, 24, 480, 432),            # x3
    #         'gbch': (53, 0, 534, 480),              # xKEGG
    ...
    #         'gba': (0, 0, 640, 427),                # xKEGG
    #         #'gbah': (80, 80, 480, 320),            # x2
    #         'gbah': (0, 0, 640, 427),               # xKEGG
    #         'arduboy': (64, 112, 512, 256),         # x4
    #     }
    elif device_name == "RG351V" or device_name == "RG351MP":
        system_viewport = {
            'standard': (1, 1, 639, 479),           # max-1
            'gb': (0, 0, 534, 480),                 # xKEGG
            'gbh': (0, 0, 534, 480),                # xKEGG
            'gbc': (0, 0, 534, 480),                # xKEGG
            'gbch': (0, 0, 534, 480),               # xKEGG
            'supervision': (0, 0, 480, 480),        # xKEGG
            'gamegear': (0, 48, 480, 432),          # xKEGG
            'ggh': (0, 48, 480, 432),               # xKEGG
            'pokemini': (0, 224, 384, 256),         # xKEGG
            'ngp': (0, 24, 480, 456),               # xKEGG
            'ngpc': (0, 24, 480, 456),              # xKEGG
            'wonderswan': (0, 96, 448, 288),        # xKEGG
            'wonderswancolor': (0, 96, 448, 288),   # xKEGG
            'gba': (0, 26, 640, 427),               # xKEGG
            'gbah': (0, 26, 640, 427),              # xKEGG
            'arduboy': (0, 224, 512, 256),          # xKEGG
            'vectrex': (0, 20, 370, 460),           # xKEGG
        }
        ...
        if not game_bezel_override or game_bezel_override == "AUTO":
            ...
            if os.path.isdir(romdir):
                with os.scandir(romdir) as it:
                    full_list = [entry.name for entry in it if entry.is_file(
                    ) and entry.name.endswith('.cfg')]
                for romname in (full_name, short_name, 'default'):
                    logger.log(f'Looking at: {romdir}/{romname}')
                    file_list = [file for file in full_list if re.fullmatch(
                        fr'{re.escape(romname)}\.[0-9]+\.cfg', file)]
                    if len(file_list) > 0:
        ...
        if os.path.isfile(bezel_system_png):
            ...
            if os.path.isdir(overlays_dir):
                for overlay_png in iglob(f'{overlays_dir}/*.png'):
                    overlay_name = os.path.splitext(
                        os.path.basename(overlay_png))[0]
                    overlay_setting = config.get_setting(
                        f'bezel.overlay.{overlay_name}')
                    if not overlay_setting:
                        continue
                    logger.log(
                        f'Adding overlay. name: {overlay_name} overlay setting: {overlay_setting}')
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(
        description="Convert ES settings from distribution.conf to RetroArch configuration in raappend.conf.")
    ...
    parser.add_argument(
        "--controllers", help="controller config", default="-p1index 0")
    parser.add_argument("--autosave", help="autosave", default="0")
    parser.add_argument("--snapshot", help="snapshot", default="")
    args = parser.parse_args()
    shader_path = set_settings(rom_name=args.rom, core=args.core, platform=args.platform,
                               controllers=args.controllers, autosave=args.autosave, snapshot=args.snapshot)
    if shader_path:
        print(shader_path)
