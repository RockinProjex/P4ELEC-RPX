...
# Flycast: Enables/Disables RTTB. Fixes certain alpha texture passing.
# Get configuration from distribution.conf and set to retrorun.cfg
get_setting "rttb"
echo "rttb:${EES}"
if [ "${EES}" == "auto" ] || [ "${EES}" == "false" ] || [ "${EES}" == "none" ] || [ "${EES}" == "0" ]; then
	if [[ "${CORE}" == "flycast" ]]; then
		sed -i "/^flycast_enable_rttb" ${RRCONF}
		echo 'flycast_enable_rttb = disabled' >> ${RRCONF}
	elif [[ "${CORE}" == "flycast2021" ]]; then
		sed -i "/^flycast2021_enable_rttb/d" ${RRCONF}
		echo 'flycast2021_enable_rttb = disabled' >> ${RRCONF}
	fi
else
        if [[ "${CORE}" == "flycast" ]]; then
		sed -i "/^flycast_enable_rttb/d" ${RRCONF}
		echo "flycast_enable_rttb = ${EES}" >> ${RRCONF}
	elif [[ "${CORE}" == "flycast2021" ]]; then
		sed -i "/^flycast2021_enable_rttb/d" ${RRCONF}
		echo "flycast2021_enable_rttb = ${EES}" >> ${RRCONF}
	fi
fi


# Flycast: Enables/Disables ALPHA SORTING.
# Get configuration from distribution.conf and set to retrorun.cfg
get_setting "alpha sorting"
echo "alpha sorting:${EES}"
if [ "${EES}" == "auto" ] || [ "${EES}" == "false" ] || [ "${EES}" == "none" ] || [ "${EES}" == "0" ]; then
	if [[ "${CORE}" == "flycast" ]]; then
		sed -i "/^flycast_enable_rttb" ${RRCONF}
		echo 'flycast_alpha_sorting = per-strip (fast, least accurate)' >> ${RRCONF}
	elif [[ "${CORE}" == "flycast2021" ]]; then
		sed -i "/^flycast2021_alpha_sorting/d" ${RRCONF}
		echo 'flycast2021_alpha_sorting = per-strip (fast, least accurate)' >> ${RRCONF}
	fi
else
        if [[ "${CORE}" == "flycast" ]]; then
		sed -i "/^flycast_alpha_sorting/d" ${RRCONF}
		echo "flycast_alpha_sorting = ${EES}" >> ${RRCONF}
	elif [[ "${CORE}" == "flycast2021" ]]; then
		sed -i "/^flycast2021_alpha_sorting/d" ${RRCONF}
		echo "flycast2021_alpha_sorting = ${EES}" >> ${RRCONF}
	fi
fi
...
# Pcsx-rearmed
# PSX Enhanced Resolution Texture Adjustment
# Get configuration from distribution.conf and set to retrorun.cfg
