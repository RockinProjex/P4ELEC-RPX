...
GL_LTIME = '/tmp/gt_launch.inf'
log_path = LOGS_DIR / 'exec.log'

# create tempory log .inf for lastgame process


def get_gamelaunch_time(platform=None, rom=None):
    t = time.time()
    current_time = int(t)

    with open(GL_LTIME, "w") as lgt_launch:
        print(str(platform) + "\n" + str(rom) + "\n" +
              str(current_time), file=lgt_launch)
...
def call_profile_func(function_name: str, *args: str) -> str:
    ...
    proc = subprocess.run(f'. /etc/profile && {shlex.quote(function_name)} {shlex.join(args)} ',
                          shell=True, stdout=subprocess.PIPE, check=True, text=True)
...
# If set to default, would equal empty string
...
def jslisten_set(*exe_names: str):
    proc = subprocess.run(
        f'. /etc/profile && jslisten set {shlex.join(exe_names)} &', shell=True,   stdout=subprocess.PIPE, check=True, text=True)
    return proc.stdout.strip('\n')
...
def check_bios(platform, core, emulator, game, log_path_):
    call_profile_func('ee_check_bios', platform,
                      core, emulator, game, log_path_)
...
def cleanup_ep128():
    ep_path = "/storage/.tmp/ep128"
    if os.path.exists(ep_path) and os.path.isdir(ep_path):
        clean_ep = 'rm -rf "' + str(ep_path) + '"'
        subprocess.run(clean_ep, shell=True)
...
def extract_archive(path: Path) -> Path:
    ...
    subprocess.check_call(['/usr/bin/7z', 'e', f'-o{str(path.parent)}', path,
                           inner_filename], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            ...
            for line in f:
                ...
                if not args[0].startswith('/'):
                    ...
                    if args[-1] == 'should_extract':
                        args = args[:-1]
                        should_extract = True
                standalone_emulators[name] = StandaloneEmulator(
                    kill_name, args, should_extract)
...
class EmuRunner():
    def __init__(self, rom: Optional[Path], platform: Optional[str], emulator: Optional[str], core: Optional[str], args: 'Mapping[str, str]') -> None:
        ...
        self.core = core
        self.args = args
        # Files that we extracted from archives, etc. to clean up later
    ...
    def toggle_max_performance(self) -> None:
        if get_elec_setting('maxperf', self.platform, self.rom.name if self.rom else None) == '1':
            ...
            call_profile_func('performance', self.platform,
                              self.rom.name if self.rom else "")
            ...
            call_profile_func('powersave', self.platform,
                              self.rom.name if self.rom else "")
            ...
            call_profile_func('custom_performance', self.platform,
                              self.rom.name if self.rom else "")
        else:
            call_profile_func('ondemand', self.platform,
                              self.rom.name if self.rom else "")
    ...
    def get_standalone_emulator_command(self) -> 'Sequence[Union[str, Path]]':
        if not self.emulator:
            raise ValueError(
                'runemu.py was called improperly, tried to launch a standard emulator with no emulator')
        ...
        command = [arg for arg in (
            path if arg == '<path>' else arg for arg in emu.args) if arg]
    ...
    def get_retroarch_command(self, shader_arg: str) -> 'Sequence[Union[str, Path]]':
        ...
        if self.emulator == 'mame':
            ...
            if rom_path:
                ...
                cmd_path = rom_path.parent.joinpath(
                    rom_path.stem.replace(' ', '_') + '.cmd')
            else:
        ...
        # decompress_ep128/64_zip and have it set the correct rom for loading here
        if self.platform == 'ep64-128' and self.rom.suffix == '.zip':
            argER = 'unzip -qd /storage/.tmp/ep128/ "' + str(self.rom) + '"'
            subprocess.run(argER, shell=True)
            # ERmod = 'chmod +x /storage/.tmp/ep128/*'
            # subprocess.run(ERmod, shell=True)
            entromc = os.popen(
                'ls /storage/.tmp/ep128/ | grep -i ".com\|.trn\|.bas\|.tap" | wc -l').read()

            if (int(entromc) > 1):
                entnosuff = str(self.rom).split('.')[0]
                entromname = str(entnosuff).split('/')[-1]
                entrom = os.popen(
                    'ls /storage/.tmp/ep128/ | grep -i "' + str(entromname) + '"').read()
            else:
                entrom = os.popen(
                    'ls /storage/.tmp/ep128/ | grep -i ".com\|.trn\|.bas\|.tap"').read()

            rom_path = '/storage/.tmp/ep128/' + str(entrom).strip()
        ...
        if self.rom and self.core == 'scummvm':
            ...
            command: 'List[Union[str, Path]]' = [
                '/usr/bin/scummvm.sh', 'libretro', self.rom]
            return command

        command: 'List[Union[str, Path]]' = [os.path.join(
            '/usr/bin/', retroarch_binary), '-L', Path('/tmp/cores/', f'{self.core}_libretro.so')]
        ...
        if 'host' in self.args or 'connect' in self.args:
            ...
            if not netplay_nick:
                netplay_nick = 'Pan4ELEC'
            if 'connect' in self.args:
                set_elec_setting('netplay.client.port', self.args['port'])
                # We should now have parsed that properly so it's just a hostname/IP address, no --port argument
                set_elec_setting('netplay.client.ip', self.args['connect'])
    ...
    def get_retrorun_command(self) -> 'Sequence[Union[str, Path]]':
        if not self.rom:
            raise ValueError(
                'runemu.py was called improperly, tried to launch retrorun with no game')
        if not self.platform:
            raise ValueError(
                'runemu.py was called improperly, tried to launch retrorun with no platform')
        if not self.core:
            raise ValueError(
                'runemu.py was called improperly, tried to launch retrorun with no platform')
    ...
    def get_mupen64plus_standalone_command(self) -> 'Sequence[Union[str, Path]]':
        if not self.rom:
            raise ValueError(
                'runemu.py was called improperly, tried to launch Mupen64Plus standalone with no video plugin')
        if not self.core:
            raise ValueError(
                'runemu.py was called improperly, tried to launch Mupen64Plus standalone with no video plugin')
        if log_level != 'minimal':
            log(
                f'Running Mupen64Plus standalone with {self.core} video plugin')
    ...
    def run(self, command: 'Sequence[Union[str, Path]]') -> None:
        ...
        with log_path.open('at', encoding='utf-8') as log_file:
            result = subprocess.run(
                command, stdout=log_file, stderr=subprocess.STDOUT, text=True, env=self.environment)
...
def main():
    ...
    get_gamelaunch_time(platform, rom)
    # make if not tools platform then...
    if platform != 'tools':
        subprocess.run('/usr/bin/r36s-speaker-on.sh', shell=True)
    ...
    try:
        screensaver = subprocess.Popen(
            ss_command, stdout=subprocess.PIPE, preexec_fn=os.setsid)
    except:
        pass
    ...
    try:
        ...
        exit_code = 1
        requires_bios = {'atari5200', 'atari800', 'atari7800', 'atarilynx', 'colecovision', 'amiga', 'amigacd32', 'o2em', 'intellivision', 'pcengine',
                         'pcenginecd', 'pcfx', 'fds', 'segacd', 'saturn', 'dreamcast', 'naomi', 'atomiswave', 'x68000', 'neogeo', 'neogeocd', 'msx', 'msx2', 'sc-3000'}
